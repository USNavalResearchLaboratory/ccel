...
double fractionOfUniquePatterns_ll(llRoot* r) {
    ...
    // Here we walk the list incrementing the uniquePatterns counter
    ...
    // after counting how many patterns are unique, we divide it by
...
/*
    Given an array containing a layer of a CCE tree calculate what
...
double fractionOfUniquePatterns_arr(unsigned int* r, unsigned long length) {
    ...
    // Here we walk the array incrementing the uniquePatterns counter
    // whenever we find a node that has only been visited once.
    for(n = 0; n < length; ++n) {
...
/* 
   Uncomment this for separate testing.
*/
/*
